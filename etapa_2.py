...
# Dados de teste
test_data = 'dados_123110470_44100hz.wav'


manchester_audio, SAMPLE_RATE_READ = sf.read(test_data) # O SAMPLE_RATE_READ deve ser 44100

# 1. Calcular o número de amostras por bit
...
manchester_audio, _ = sf.read(test_data)
...
for i in range(0,60):
    ...
    noisy_signal = adicionar_ruido(manchester_audio, snr)
    decoded = decode_nrz(noisy_signal, num_bits)

    for j in range(len(decoded_manchester)):
        if (decoded_manchester[j] != decoded[j]):
            
            numero_erros += 1

    print(f"valor snr: {snr}")
    print(f"numero de erros: {numero_erros}")

print(f"  Original: {decoded_manchester}")
print(f"  Decodificado: {decoded}")
print(f"  Correto: {decoded_manchester == decoded}\n")
